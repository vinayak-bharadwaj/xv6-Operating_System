...
static uint time_counter = 0; //universal counter
int nextpid = 1;
...
found:
  ...
  p->pid = nextpid++;
  // p->va_set = false;
  p->va = 0x00000000; // dummy virtual address is assigned.
...
resetProcessPagesData(struct proc * p)
{
  for(int i = 0; i < MAX_TOTAL_PAGES; i++)
  {
    ...
    p->paging_meta_data[i].pg.pagestate = 0;                  // Page state
  ...
  p->num_pages_in_main_mem = 0;                           // Initialize number of pages stored in physical memory
  p->head_page_in_mem = 0;
}
void swapin(void);
...
userinit(void)
{
  ...
  // Calling the kernel processes
  
  create_kernel_process((char *)"swapin\0", swapin);
  cprintf("Hello\n");
  create_kernel_process((char *)"swapout\0", swapout);
}
...
void swapin(void){
  cprintf("swapin called\n");
}

void swapout(void){
  cprintf("swapout called here\n");
}
...
void create_kernel_process(const char *name, void(*entrypoint)()){
  ...
  if((kernel_p->pgdir = setupkvm()) == 0){ //in turn calls mappages for virtual to physical address maping.
    panic("out of memory?");
  }
...
void update_recently_accessed(void){
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (p && (p->pid > 2) && (p->state != UNUSED)  && (p->state != EMBRYO) && (p->state != ZOMBIE))
    {
      ...
      while(head){
        if (head->pg.pagestate == 1){
          pte = walkpgdir(p->pgdir, (void *) head->pg.virtualAddr, 0);
    ...
    //     if (pageToUpdate->pg.pagestate == 1)
    //     { 
